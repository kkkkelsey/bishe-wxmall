...
public class CartInfoService {
    ...
    public CartInfo add(CartInfo detailInfo){
        ...
        //先查询购物车里有没有该商品，有就更新数量+1，没有就添加
        Example example = new Example(CartInfo.class);
        example.createCriteria().andEqualTo("userid",userId).andEqualTo("goodsid",goodsId); //查询条件为字段等于value值
        List<CartInfo> infos = cartInfoMapper.selectByExample(example);  //根据条件查询返回list
        if (CollectionUtil.isEmpty(infos)){
            ...
            //更新
            CartInfo cartInfo = infos.get(0);  //获取购物车信息
            cartInfo.setCount(cartInfo.getCount() + detailInfo.getCount());  //商品数量+1
            cartInfoMapper.updateByPrimaryKeySelective(cartInfo);  //更新购物车信息
        }
        return detailInfo;
    }
    ...
    public List<GoodsInfo> findAll(Long userId){
        ...
        for (CartInfo cartInfo:cartInfoList){
            ...
            if (goodsInfo != null){
                ...
                goodsList.add(goodsInfo);  //将商品信息存入商品数组
            }
        }
        return goodsList;
    }
